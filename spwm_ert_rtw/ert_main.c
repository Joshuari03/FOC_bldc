...
extmodeSimulationTime_T getCurrentTaskTime(int tid)
{
  ...
  switch (tid) {
   case 2:
    currentTime = (extmodeSimulationTime_T) ((spwm_M->Timing.clockTick2) * 0.01
      + 0.005);
    break;
  }
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(spwm_M) == (NULL));
  while (runModel) {
    ...
    if (rtmStepTask(spwm_M, 2)
        ) {
      mw_osSemaphoreRelease(&subrateTaskSem[0]);
    }

    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      ((spwm_M->Timing.clockTick1) * 0.001);
    ...
    stopRequested = !((rtmGetErrorStatus(spwm_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(spwm_M, "stopping the model");
}
